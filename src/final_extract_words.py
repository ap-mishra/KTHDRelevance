...
if __name__=='__main__':
    #file_splitter("../data/input/train.tmp3")
    nose = range(1,741,1)
    for number in nose:
        train = pd.read_csv("../data/input/CHUNKS/x_"+str(number),sep=",")

#    for no in range(1,50,1):
#        train = pd.read_csv("../data/input/CHUNKS/file_chunk"+str(no)+".txt",sep=",")
#        train2 = create_word_features(train)
#        train2 = train2.append(train2)
#
#    train2.to_csv("../data/output/FINALTRAIN.csv",index=False)
        print "Processing chunk"+str(number)
        train2 = create_word_features(train)
        train2.to_csv("../data/output/CHUNKS/train_"+str(number)+".csv", index=False)
    print train2.head()
